...
def show_update_detail():
    ...
    if request.method=='POST':
        ...
        patient_query = "select Patient_ID, organ_req, reason_of_procurement, Doctor_name from Patient inner join Doctor on Doctor.Doctor_ID = Patient.Doctor_ID and User_ID = %s" %(request.form['User_ID'])
        donor_query = "select Donor_ID, organ_donated, reason_of_donation, Organization_name from Donor inner join Organization on Organization.Organization_ID = Donor.Organization_ID and User_ID = %s" %(request.form['User_ID'])
        transaction_query = "select distinct Transaction.Patient_ID, Transaction.Donor_ID, Organ_ID, Date_of_transaction, Status from Transaction, Patient, Donor where (Patient.User_ID = %s and Patient.Patient_ID = Transaction.Patient_ID) or (Donor.User_Id= %s and Donor.Donor_ID = Transaction.Donor_ID)" %((request.form['User_ID']),(request.form['User_ID']))
        #
        patient_results = ()
        donor_results = ()
        transaction_results = ()
        mycursor.execute(patient_query)
        if(mycursor.rowcount > 0):
            patient_results = mycursor.fetchall()
        fields_pat = mycursor.column_names
        #
        mycursor.execute(donor_query)
        if(mycursor.rowcount > 0):
            donor_results = mycursor.fetchall()
        ...
        if("show" in request.form):
            return render_template('show_detail_2.html',res = res,fields = fields, not_found=not_found, phone_no = phone_no, donor_results = donor_results, patient_results = patient_results,transaction_results = transaction_results,fields_trans = fields_trans, fields_dnr = fields_dnr, fields_pat = fields_pat)
